...
function reverseGeocodeNominatim(float $lat, float $lng, int $zoom): ?string {
    ...
    // Use house number if available
    if (isset($address['house_number']) && $address['house_number']) {
        if (!empty($roadParts)) {
            // E.g. "Jl. Telekomunikasi No. 1"
            $parts[] = implode(' ', $roadParts) . ' No. ' . $address['house_number'];
        } else {
            $parts[] = 'No. ' . $address['house_number'];
        }
    } elseif (!empty($roadParts)) {
    ...
    // 4. Kelurahan/Desa (village, suburb)
    $kelurahan = '';
    if (isset($address['village']) && $address['village']) {
        $kelurahan = $address['village'];
    } elseif (isset($address['suburb']) && $address['suburb'] && !preg_match($rtRwPattern, $address['suburb'])) {
        $kelurahan = $address['suburb'];
    } elseif (isset($address['neighbourhood']) && $address['neighbourhood'] && !preg_match($rtRwPattern, $address['neighbourhood'])) {
    ...
    if ($kelurahan) {
        // Add "Kel." prefix if not present
        if (stripos($kelurahan, 'Desa') === false && stripos($kelurahan, 'Kelurahan') === false && stripos($kelurahan, 'Kel.') === false) {
             $parts[] = "Kel. " . $kelurahan;
        } else {
             $parts[] = $kelurahan;
        }
    ...
    // 5. Kecamatan (town can be kecamatan if county exists)
    $kecamatan = '';
    if (isset($address['city_district']) && $address['city_district']) {
        $kecamatan = $address['city_district'];
         ...
         $kecamatan = $address['municipality'];
    } elseif (isset($address['town']) && $address['town'] && (isset($address['city']) || isset($address['county']))) {
         // If town exists AND (city OR county) exists, treat town as Kecamatan
         $kecamatan = $address['town'];
    }
    
    if ($kecamatan) {
        if (stripos($kecamatan, 'Kecamatan') === false && stripos($kecamatan, 'Kec.') === false) {
            $parts[] = "Kec. " . $kecamatan;
        } else {
            $parts[] = $kecamatan;
        }
    ...
    if (isset($address['city']) && $address['city']) {
        ...
        $city = $address['county'];
    } elseif (isset($address['town']) && $address['town'] && !$kecamatan) {
        // Fallback: if town used but NOT as kecamatan (i.e. no city/county), use as city
        $city = $address['town'];
    }
    ...
    if (!empty($parts)) {
        ...
        error_log("Reverse geocoding success (Enhanced): $detailedAddress");
        return $detailedAddress;
    }
...
if ($action === 'get_ultra_detailed_stats' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'export_kpi') {
        ...
        if ($filterType === 'monthly') {
            $year = (int)($_GET['year'] ?? date('Y'));
            $month = (int)($_GET['month'] ?? date('n'));
            // Standardize on 2024-01-01 format
            $periodStart = sprintf('%04d-%02d-01', $year, $month);
            $periodEnd = date('Y-m-t', strtotime($periodStart));
        } else {
            // Period mode (default)
            // Note: getAllKPIData handles default period internally if null passed
        }
        ...
        header('Content-Disposition: attachment; filename="export_kpi_' . date('Y-m-d') . '.csv"');
        ...
        // Header
        fputcsv($out, [
            'No', 'Nama', 'Hari Kerja (Total)', 'Hari Kerja (Aktual)', 
            'Tepat Waktu', 'WFA', 'Terlambat (kali)', 'Terlambat (menit)', 
            'Izin/Sakit', 'Alpha', 'Lembur', 'Laporan Harian Kosong', 
            'Score', 'Grade'
        ]);
        ...
        foreach ($data['kpi_data'] as $row) {
            fputcsv($out, [
                ...
                $row['wfa_count'] ?? 0,
                $row['late_count'],
                $row['total_late_minutes'], // In minutes
    ...
    if ($action === 'export_daily') {
        ...
        $stmt = $pdo->prepare("
            SELECT dr.*, u.nama, u.startup 
            FROM daily_reports dr 
            JOIN users u ON u.id = dr.user_id 
            WHERE dr.report_date BETWEEN :s AND :e 
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="export_harian_' . $start . '_to_' . $end . '.csv"');
        
        $out = fopen('php://output', 'w');
        fprintf($out, chr(0xEF).chr(0xBB).chr(0xBF));
        
        fputcsv($out, ['Tanggal', 'Nama', 'Startup', 'Isi Laporan', 'Status', 'Evaluasi']);
    ...
    if ($action === 'export_monthly') {
        ...
        if ($term) {
            $sql .= " AND (u.nama LIKE :t OR u.nim LIKE :t)";
            $params[':t'] = "%$term%";
        }
        
        $sql .= " ORDER BY mr.year DESC, mr.month DESC, u.nama ASC";
        ...
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="export_bulanan_' . ($year ? $year : 'all') . '.csv"');
        
        $out = fopen('php://output', 'w');
        fprintf($out, chr(0xEF).chr(0xBB).chr(0xBF));
        
        fputcsv($out, ['Tahun', 'Bulan', 'NIM', 'Nama', 'Startup', 'Ringkasan', 'Status']);
